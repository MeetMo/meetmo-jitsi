...
import { setLayout, setBackground } from '../letxsoft/actions.web';
...
function _onFollowMeCommand(attributes = {}, id, store) {
    ...
    // In current case of the project, The moderator is hidden
    // And hidden participant does not exists at remote side
    const participantSendingCommand = getParticipantById(state, id);

    if (participantSendingCommand !== undefined) {
    ...
    // Set the current layout of the conference.
    if (oldState.currentLayout !== attributes.currentLayout) {
        store.dispatch(setLayout(attributes.currentLayout));
    }

    // Set the current layout of the conference.
    if (oldState.currentBackground !== attributes.currentBackground) {
        store.dispatch(setBackground(attributes.currentBackground));
    }
