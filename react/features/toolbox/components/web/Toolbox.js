...
import { LayoutPopup, BackgroundPopup } from '../../../letxsoft';
import {
...
import { toggleSharedVideo, toggleSharedVimeoVideo } from '../../../shared-video';
...
type Props = {
    ...
    /**
     * Whether or not the local participant is sharing a Vimeo video.
     */
    _sharingVimeoVideo: boolean,
    ...
    /**
     * If the local user is moderator.
     */
...
class Toolbox extends Component<Props, State> {
    ...
    constructor(props: Props) {
        ...
        this._onToggleLayoutPopup = this._onToggleLayoutPopup.bind(this);
        this._onToggleBackgroundPopup = this._onToggleBackgroundPopup.bind(this);
    ...
    /**
     * Dispatches an action to toggle Vimeo video sharing.
     *
     * @private
     * @returns {void}
     */
    ...
    */
    ...
    _onToolbarToggleVimeoSharedVideo: () => void;

    /**
     * Creates an analytics toolbar event and dispatches an action for toggling
     * the sharing of a Vimeo video.
     *
     * @private
     * @returns {void}
     */
    _onToolbarToggleVimeoSharedVideo() {
        sendAnalytics(createToolbarEvent('shared.vimeoVideo.toggled',
            {
                enable: !this.props._sharingVimeoVideo
            }));
    ...
    _renderDesktopSharingButton(isInOverflowMenu = false) {
        ...
        const iconShareDesktop = !isEmpty(iconDataShareDesktop) ? (
            <IconFromConfig configuration={iconDataShareDesktop} />
        ) : (
            IconShareDesktop
        );
    ...
    _renderOverflowMenuContent() {
        ...
        const iconFullScreen = !isEmpty(iconDataFullscreen) ? (
            <IconFromConfig configuration={iconDataFullscreen} />
        ) : (
            IconFullScreen
        );

        const iconExitFullScreen = !isEmpty(iconDataFullscreen) ? (
            <IconFromConfig configuration={iconDataFullscreen} />
        ) : (
            IconExitFullScreen
        );
    ...
    /**
     * Renders the buttons for the layout change popup.
     *
     * @inheritdoc
     */
    _renderLayoutButton() {
    ...
    /**
     * Renders the buttons for the Background change popup.
     *
     * @inheritdoc
     */
    ...
    _onToggleBackgroundPopup: () => void;

    /**
     * Toggles the Background popup.
     *
     * @inheritdoc
     */
    _onToggleBackgroundPopup() {
        this.props.dispatch(openDialog(BackgroundPopup));
    }

    _onToggleLayoutPopup: () => void;

    /**
     * Toggles the Layout popup.
     *
     * @inheritdoc
     */
    _onToggleLayoutPopup() {
        this.props.dispatch(openDialog(LayoutPopup));
    }
        ...
        return movedButtons.map(buttonName => {
            switch (buttonName) {
                ...
                case 'chat':
                    ...
                    const iconChat = !isEmpty(iconDataForChat) ? (
                        <IconFromConfig configuration={iconDataForChat} />
                    ) : (
                        IconChat
                    );
                ...
                case 'invite':
                    ...
                    const iconInviteMore = !isEmpty(iconDataInvitePeople) ? (
                        <IconFromConfig configuration={iconDataInvitePeople} />
                    ) : (
                        IconInviteMore
                    );
...
function _mapStateToProps(state) {
    ...
    const sharedVimeoVideoStatus = state['features/shared-video'].vimeoStatus;
    const {
        fullScreen,
        overflowMenuVisible
    ...
    return {
        ...
        _sharingVimeoVideo: sharedVimeoVideoStatus === 'playing'
            || sharedVimeoVideoStatus === 'start'
            || sharedVimeoVideoStatus === 'pause',
        _visible: isToolboxVisible(state),
        _visibleButtons: equals(visibleButtons, buttons) ? visibleButtons : buttons,
        _isModerator: Boolean(localParticipant.role === 'moderator')
    };
}
