    ...
    toggleSharedVideo() {
        ...
        if (!this.isSharedVideoShown) {
            requestVideoLink().then(
                    url => {
                        this.emitter.emit(
                            UIEvents.UPDATE_SHARED_VIDEO, url, 'start', null, null, null, 'youtube');
        ...
        if (APP.conference.isLocalId(this.from)) {
            showStopVideoPropmpt().then(
                () => {
                    ...
                    this.emitter.emit(
                        UIEvents.UPDATE_SHARED_VIDEO, this.url, 'stop', null, null, null, 'youtube');
    ...
    fireSharedVideoEvent(sendPauseEvent) {
        ...
        if (state === YT.PlayerState.PAUSED && sendPauseEvent) {
            this.emitter.emit(UIEvents.UPDATE_SHARED_VIDEO,
                this.url, 'pause', this.player.getCurrentTime(), null, null, 'youtube');
    ...
    onSharedVideoStop(id, attributes) {
        ...
        VideoLayout.showLargeVideoContainer(SHARED_VIDEO_CONTAINER_TYPE, false)
            .then(() => {
                ...
                this.emitter.emit(
                    UIEvents.UPDATE_SHARED_VIDEO, null, 'removed', null, null, null, 'youtube');
            });
