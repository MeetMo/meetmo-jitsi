...
import SharedVimeoVideoManager from './shared_video/SharedVimeoVideo';
...
const UIListeners = new Map([
    [
        ...
        UIEvents.SHARED_VIMEO_VIDEO_CLICKED,
        () => sharedVimeoVideoManager && sharedVimeoVideoManager.toggleSharedVideo()
    ]
]);
...
/**
 * Returns true if there is a shared vimeo video which is being shown (?).
 * @returns {boolean} - true if there is a shared video which is being shown.
 */
UI.isSharedVimeoVideoShown = function() {
    return Boolean(sharedVimeoVideoManager && sharedVimeoVideoManager.isSharedVideoShown);
};
 ...
 * @return {EtherpadManager} the shared document manager object
 */
UI.getSharedVimeoVideoManager = function() {
    return sharedVimeoVideoManager;
};
...
UI.start = function() {
    ...
    sharedVimeoVideoManager = new SharedVimeoVideoManager(eventEmitter);
...
UI.onSharedVideoStart = function(id, url, attributes) {
    if (attributes && attributes.type && attributes.type === 'vimeo') {
        sharedVimeoVideoManager && sharedVimeoVideoManager.onSharedVideoStart(id, url, attributes);
    } else {
        sharedVideoManager && sharedVideoManager.onSharedVideoStart(id, url, attributes);
    }
};
...
UI.onSharedVideoUpdate = function(id, url, attributes) {
    if (attributes && attributes.type && attributes.type === 'vimeo') {
        sharedVimeoVideoManager && sharedVimeoVideoManager.onSharedVideoUpdate(id, url, attributes);
    } else {
        sharedVideoManager && sharedVideoManager.onSharedVideoUpdate(id, url, attributes);
    }
};
...
UI.onSharedVideoStop = function(id, attributes) {
    if (attributes && attributes.type && attributes.type === 'vimeo') {
        sharedVimeoVideoManager && sharedVimeoVideoManager.onSharedVideoStop(id, attributes);
    } else {
        sharedVideoManager && sharedVideoManager.onSharedVideoStop(id, attributes);
    }
};
